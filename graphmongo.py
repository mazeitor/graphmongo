...
class GraphMongo(MongoClient, list):
    ...
    _schema = {
        "_id":      {"name": "id", "type": "string", "for": ["node", "edge"]},
    ...
    def __Get(self, elems=None, type=None, query=None):
        ...
        '''
        try:
            # at the moment the query result applied is or by list of id's or
            # the query passed as a parameter
            if query is None:
                query = {}
    ...
    def GetNeighbours(self,nodes=None,edges=None,label=None,weight=None,direction=None,query=None,disjunction=None):
        ...
        aux = self._CopyObject()  # pipeline method
        aux.SetParameters(elems=elems, pipetype=self._pipetype)
        aux._accumulated = list(set(aux._accumulated) | set(nodes))
                                # we accumulated nodes and edges without
                                # distinction
    ...
    def __GetNodeNeighbours(self,nodes=None,edges=None,label=None,weight=None,query=None,direction=None):
        ...
        # direction have node_b be "node_a" or "node_b" depending if we want
        # node_b retrieve node_a or node_b
        ...
        try:
            ...
            if nodes is None and edges is not None:
                ...
                elems = self[self._ddbb][
                    self._edge].aggregate([match, project])
                ...
                if query:
                    query = {
                        "$and": [{"_id": {"$in": edges}}, {"{0}._id".format(FROM): {"$in": nodes}}, query]}
                else:
                    query = {
                        "$and": [{"_id": {"$in": edges}}, {"{0}._id".format(FROM): {"$in": nodes}}]}
                match = {"$match": query}
                project = {"$project": {"_id": "${0}._id".format(TO)}}
                elems = self[self._ddbb][
                    self._edge].aggregate([match, project])
                ...
                if query is not None:
                    if nodes:
                        query = {
                            "$and": [{"{0}._id".format(FROM): {"$in": nodes}}, query]}
                    match = {"$match": query}
                    project = {"$project": {"_id": "${0}._id".format(TO)}}
                    elems = self[self._ddbb][
                        self._edge].aggregate([match, project])
                    ...
                    match = {
                        "$match": {"{0}._id".format(FROM): {"$in": nodes}}}
                    project = {"$project": {"_id": "${0}._id".format(TO)}}
                    elems = self[self._ddbb][
                        self._edge].aggregate([match, project])
            ids = [elem["_id"] for elem in elems]
...
def Queries(name):
    ...
    relatednodes = nodes.GetNeighbours()
    relatednodes = relatednodes.GetNeighbours()
                                              # add disjunction, remove
                                              # previous nodes
    print relatednodes[:]
    ...
    relatednodes = nodes.GetNeighbours()
    relatednodes = relatednodes.GetNeighbours(
        disjunction=["node"])  # add disjunction, remove previous nodes
    print relatednodes[:]
